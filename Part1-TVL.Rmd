...
#Store the R^2 of the models in a list 
r2 = vector("double", l)
for(i in 1:l){
  r2[[i]] = summary(protocols$model[[i]])$r.squared
}

names(r2) = protocolsNames

protocols = cbind(protocols, r2)
...
As we guessed **PancakeSwap** *TVL* is better correlated with *BNB*, indeed $R^2\approx0.93$.
...
protocols = protocols %>%
  select(name, tvl, plot1, plot2, plot3, plot4, plot5, dygraphsEth, dygraphsNoEth, model, r2)
