...
```
```{r genDygraphs}
genDygraphsNoEth = function(name){
  xts(x = protocols$tvl[[name]][c(1, 2)], order.by = protocols$tvl[[name]]$date)
}

genDygraphsEth = function(name){
  xts(x = protocols$tvl[[name]][c(1, 2, 3)], order.by = protocols$tvl[[name]]$date)
}

protocols = mutate(protocols, dygraphsEth = map(name, genDygraphsEth))
names(protocols$dygraphsEth) = protocolsNames
protocols = mutate(protocols, dygraphsNoEth = map(name, genDygraphsNoEth))
names(protocols$dygraphsNoEth) = protocolsNames
```
...
genPlot3 = function(name){
  ggplot(data = protocols$tvl[[name]]) +
    geom_point(aes(ethPrice, totalLiquidityUSD), color="darkblue")+
...
plots[[7]] = pswap %>%
  ggplot()+
  geom_line(aes(date, derivative, color="Derivative")) + 
  ...
  theme_bw()+
  scale_color_manual(values = c("Derivative" = "red", "Balance" = "Blue"))
...
protocols = protocols %>%
  select(name, tvl, plot1, plot2, plot3, plot4, dygraphsEth, dygraphsNoEth, model, p)
